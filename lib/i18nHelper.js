...
(function (global) {
  ...
  const SUPPORTED_FIXED_LANGS = ["en", "es", "de", "fr"];

  function getFixedDisplayLang(lang) {
    return SUPPORTED_FIXED_LANGS.includes(lang) ? lang : "browser";
  }
  ...
  async function initI18n() {
    try {
      ...
      const fixedLang = getFixedDisplayLang(stored.displayLanguage);
      displayLang = fixedLang;
      if (fixedLang === "browser") {
        messageMap = null;
        return;
      }
      const url = api.runtime.getURL("_locales/" + fixedLang + "/messages.json");
