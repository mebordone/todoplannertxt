...
function buildBuiltInFsaApi() {
  ...
  return {
    ...
    async getFolderWithPicker(options) {
      const opts = options && options.displayPath != null ? { displayPath: options.displayPath } : undefined;
      const result = await api.getFolderWithPicker(opts);
      if (result && result.error) throw new Error(result.error);
      return (result && result.folderPath != null) ? { folderPath: result.folderPath } : null;
    },
...
function getFsaApi() {
  return useBuiltInFileAccess() ? buildBuiltInFsaApi() : buildProxyFsaApi();
}

const fsaApi = getFsaApi();
...
/** Re-check if built-in file access is available (e.g. after experiment loads) and update globals. */
function ensureBuiltInApi() {
  const api = typeof browser !== "undefined" ? browser : chrome;
  const selfObj = typeof globalThis !== "undefined" ? globalThis : self;
  if (api && api.todotxtFileAccess && !selfObj.fsaUsesBuiltIn) {
    selfObj.fsaApi = buildBuiltInFsaApi();
    selfObj.fsaUsesBuiltIn = true;
  }
}
(typeof globalThis !== "undefined" ? globalThis : self).ensureBuiltInApi = ensureBuiltInApi;
