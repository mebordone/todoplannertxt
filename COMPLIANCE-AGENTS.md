...
| Calendario → todo (`todoLineId`) | Tests unitarios / integración | ✅ `calendarMappings.calendarItemPayloadToTodoPlain` (equivalente al armado en `calendarAdapter.calendarItemToTodoPlain`) cubierto en `test/unit/calendarMappings.test.js`. Reintento `modifyItem`/`deleteItem` tras refrescar disco en `test/integration/todoclient.test.js`; el mock FSA devuelve `done.txt` vacío para alinear con `fileUtil.readTodo` (todo + done concatenados). |
