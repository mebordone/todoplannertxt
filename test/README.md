...
- **test/unit/** – Tests funcionales por módulo (todotxt, util, md5, exception, fileUtil) y por helpers de `lib/` incluidos en cobertura (p. ej. `plainItemDueUpdate`, `taskDuePickerLayout`, `monochromeCalendarIcon`, `extensionUiRoutes`, `priorityDisplay`, `weekRange`, `quickAddToken`, `pendingOnlyFilter`).
